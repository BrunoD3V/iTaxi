...
if($novapass == $reppass){
	...
	if(!password_verify($passantiga, $hashAndSalt))
		...
		$sthandler = $handler->prepare("UPDATE clientes SET password = :password  WHERE username = :username");
	...
	echo 'window.location.replace("http://http://193.137.101.50/webdev2016/alunos/bruno/web/areaclientePage.php");';
	echo '</script>';
}
